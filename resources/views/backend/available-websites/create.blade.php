...
@section('title', 'Create Website - NexusAdmin')
@section('page-title', 'Create Available Website')
...
<div style="max-width:760px;">
    <div style="display:flex; align-items:center; gap:0.75rem; margin-bottom:1.5rem;">
        ...
        <div>
            <h2 style="font-size:1.25rem; font-weight:600; color:var(--text-primary);">Create Website</h2>
            <p style="font-size:0.875rem; color:var(--text-secondary); margin-top:0.2rem;">Add a new website entry in backend panel.</p>
        </div>
    </div>

    <form action="{{ route('available-websites.store') }}" method="POST">
        @csrf
        <div class="card" style="border-radius:12px; background:var(--card-bg); border:1px solid var(--border-color); padding:1.5rem; margin-bottom:1.25rem;">
            <div style="display:grid; grid-template-columns:repeat(2, minmax(0, 1fr)); gap:1rem;">
                ...
                <div style="grid-column:1 / -1;">
                    ...
                    <input type="url" name="url" value="{{ old('url') }}" placeholder="https://example.com" style="width:100%; padding:0.625rem 0.875rem; border-radius:8px; border:1px solid var(--border-color); background:var(--input-bg,#fff); color:var(--text-primary); font-size:0.9rem; box-sizing:border-box;">
                ...
                <div>
                    ...
                    <input type="text" name="icon" value="{{ old('icon') }}" placeholder="language" style="width:100%; padding:0.625rem 0.875rem; border-radius:8px; border:1px solid var(--border-color); background:var(--input-bg,#fff); color:var(--text-primary); font-size:0.9rem; box-sizing:border-box;">
                </div>
                <div>
        ...
        <div style="display:flex; gap:0.75rem;">
            <button type="submit" class="btn btn-primary" style="display:inline-flex; align-items:center; gap:0.4rem;"><span class="material-icons-round" style="font-size:1.1rem;">save</span> Create Website</button>
